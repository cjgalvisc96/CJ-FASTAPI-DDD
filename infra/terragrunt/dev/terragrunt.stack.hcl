# =============================================================================
# Environment stack (this file is identical for dev / prod / local — it auto-loads
# THIS directory's env.hcl, which is the single source of per-env truth).
#
# Declares the 7 units of the platform ONCE and instantiates them with this env's
# knobs sourced from ./env.hcl. The unit WIRING (module source, dependencies,
# mock_outputs, input plumbing) lives in ../units/<component>/terragrunt.hcl and is
# shared by every environment — change wiring there once, not per env.
#
# Usage (run from this directory, e.g. infra/terragrunt/dev):
